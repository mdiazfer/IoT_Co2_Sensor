...
/**
 * @brief Get the service Map of this characteristic.
 * @param [in] N/A
 * @return Pointer to the service map
 */
BLEServiceMap* BLEServer::getServiceMap() {
	log_v(">> getServiceMap");
	log_v("<< getServiceMap");
	return &m_serviceMap;
} // getServiceMap
...
/**
 * @brief Refresh in the ESP routine all the services previously created.
 * @param [in] N/A
 * @return N/A.
 */

void BLEServer::recreateServerGATTTable() {
	log_v(">> recreateServerGATTTable() - Recreate all the services");

	//Register the appId of this server
	registerApp(m_appId);
	...
	while (pService != nullptr) {
		pService->recreateServiceGATTTable(this);
		pService = m_serviceMap.getNext();
	} // End while

	log_v("<< recreateServerGATTTable");
} // recreateServerGATTTable
...
/**
 * @brief Add the Service that has been created before
 * @param [in] pService
 * @return Error (fasle) or Success (true)
*/
bool BLEServer::addService(BLEService* pService) {
	if (pService==nullptr) return false;
	
	BLEUUID uuid=pService->getUUID();

	log_v(">> addService - %s", uuid.toString().c_str());
	m_semaphoreCreateEvt.take("addService");

	m_serviceMap.setByUUID(uuid, pService); // Save a reference to this service being on this server.
	pService->executeCreate(this);          // Perform the API calls to actually create the service.

	m_semaphoreCreateEvt.wait("addService");
...
/**
 * @brief Get the server callbacks.
 * 
 * @result [out] pCallbacks The callbacks to be invoked.
 */
BLEServerCallbacks* BLEServer::getCallbacks(void) {
