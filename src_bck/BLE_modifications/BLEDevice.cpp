...
void BLEDevice::removeAdvertising() {
	if(m_bleAdvertising == nullptr) {
		log_i("advertising is already null");
		return ;
	}

	free(m_bleAdvertising);
	m_bleAdvertising=nullptr;
	log_i("remove advertising");
	return ;
}
...
/**
 * @brief Get a pointer of the current instance of the server.
 * @return The pointer to the instance of the server.
 */
