...
/**
 * @brief Refresh in the ESP routine a characteristic previously created.
 * @param [in] pService The service with which to associate this characteristic.
 */
void BLECharacteristic::recreateCharacteristicGATTTable(BLEService* pService) {
	log_v(">> recreateCharacteristicGATTTable()");

	if (m_handle == NULL_HANDLE) {
		log_e("Characteristic has no handle.");
		return;
	}
	...
	m_semaphoreCreateEvt.take("recreateCharacteristicGATTTable");
	esp_err_t errRc = ::esp_ble_gatts_add_char(
		m_pService->getHandle(),
		getUUID().getNative(),
		static_cast<esp_gatt_perm_t>(m_permissions),
	...
	m_semaphoreCreateEvt.wait("recreateCharacteristicGATTTable");

	BLEDescriptor* pDescriptor = m_descriptorMap.getFirst();
	while (pDescriptor != nullptr) {
		pDescriptor->recreateDescriptorGATTTable(this);
		pDescriptor = m_descriptorMap.getNext();
	} // End while

	log_v("<< recreateCharacteristicGATTTable");
} // recreateCharacteristicGATTTable


/**
 * @brief Get the descriptor Map of this characteristic.
 * @param [in] N/A
 * @return Pointer to the descriptor map
 */
BLEDescriptorMap* BLECharacteristic::getDescriptorMap() {
	log_v(">> getDescriptorMap");
	log_v("<< getDescriptorMap");
 ...
 * @result [out] pCallbacks a pointer of a callbacks structure used to define any callbacks for the characteristic.
 */
BLECharacteristicCallbacks* BLECharacteristic::getCallbacks(void) {
	log_v(">> getCallbacks:");
	log_v("<< getCallbacks");
	return m_pCallbacks;
} // getCallbacks
