 ...
 * @param [in] pCharacteristic The characteristic to which to register this descriptor.
 */
void BLEDescriptor::recreateDescriptorGATTTable(BLECharacteristic* pCharacteristic) {
	log_v(">> recreateDescriptorGATTTable(): %s", toString().c_str());

	if (m_handle == NULL_HANDLE) {
		log_e("Descriptor has no handle.");
		return;
	}

	m_pCharacteristic = pCharacteristic; // Save the characteristic associated with this service.

	/*log_v(">> Wait 5 seconds before taking the semaphore");
	for (int i=0;i<25;i++) delay(200); //Wait 5ssecond
	log_v(">> Wait done");*/
	
	esp_attr_control_t control;
	control.auto_rsp = ESP_GATT_AUTO_RSP;
	//m_semaphoreCreateEvt.take("recreateDescriptorGATTTable");
	esp_err_t errRc = ::esp_ble_gatts_add_char_descr(
			pCharacteristic->getService()->getHandle(),
			getUUID().getNative(),
			(esp_gatt_perm_t)m_permissions,
	...
	//m_semaphoreCreateEvt.wait("recreateDescriptorGATTTable");
	log_v("<< recreateDescriptorGATTTable");
} // recreateDescriptorGATTTable
