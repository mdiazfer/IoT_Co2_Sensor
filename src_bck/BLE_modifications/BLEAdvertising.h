...
public:
	...
	char* getPtrData(); 										// Retrieve pointer to the current advert payload.
	size_t 		getSizeData();
...
public:
	...
	std::vector<BLEUUID> vectorServiceUUIDs(void);

	std::string 					getData();
  char* 								getPtrData();
	size_t 								getSizeData();
	esp_ble_adv_data_t*   getPtrAdvData();
	esp_ble_adv_data_t*   getPtrScanRespData();
	size_t 								getSizeParam();
	esp_ble_adv_params_t* getPtrAdvParams();
	bool*                 getCustomAdvData();
	bool*                 getCustomScanResponseData();
	bool*                 getScanResp();
