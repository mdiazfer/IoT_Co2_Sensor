...
/**
 * @brief Refresh in the ESP routine a service previously created.
 * @param [in] pServer The server with which to associate this service.
 * @return N/A.
 */

void BLEService::recreateServiceGATTTable(BLEServer* pServer) {
	log_v(">> recreateServiceGATTTable() - Recreate service (esp_ble_gatts_create_service) service uuid: %s", getUUID().toString().c_str());

	if (m_handle == NULL_HANDLE) {
		log_e("Service has no handle.");
		return;
	}

	m_pServer          = pServer;
	m_semaphoreCreateEvt.take("recreateServiceGATTTable"); // Take the mutex and release at event ESP_GATTS_CREATE_EVT
	...
	m_semaphoreCreateEvt.wait("recreateServiceGATTTable");

	//Recreate GATT table also for all the characteristics linked to this service
	BLECharacteristic* pCharacteristic = m_characteristicMap.getFirst();
	while (pCharacteristic != nullptr) {
		pCharacteristic->recreateCharacteristicGATTTable(this);
		pCharacteristic = m_characteristicMap.getNext();
	} // End while

	//Now start the service
	log_v(">> startService()");
	m_semaphoreStartEvt.take("startService");
	errRc = ::esp_ble_gatts_start_service(m_handle);
	log_v("<< esp_ble_gatts_start_service done with result=%d %s",errRc, GeneralUtils::errorToString(errRc));

	if (errRc != ESP_OK) {
		log_e("<< esp_ble_gatts_start_service: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
		return;
	}
	m_semaphoreStartEvt.wait("startService");

	log_v("<< startService()");
	log_v("<< recreateServiceGATTTable");
} // recreateServiceGATTTable
...
void BLEService::setHandle(uint16_t handle) {
	...
	if (m_handle != NULL_HANDLE) {
		log_d("!!! Handle is already set %.2x", m_handle);
		return;
	}
	m_handle = handle;
	log_v("<< setHandle");
...
/**
	 * @brief Get the characteristic Map of this service.
	 * @param [in] N/A
	 * @return Pointer to the characteristic map
	 */
	BLECharacteristicMap* BLEService::getCharacteristicMap() {
		log_v(">> getCharacteristicMap");
		log_v("<< getCharacteristicMap");
